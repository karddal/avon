...
describe("Calendar e2e test", () => {
  ...
  it("switching to events triggers another events fetch with valid range", () => {
    ...
    cy.then(() => {
      ...
      cy.then(() => {
        expect(
          eventsCalls.length,
          "events fetch after switching to events tab",
        ).to.eq(before + 1);
      });
      ...
      cy.wait("@getEvents").then((i) => {
        ...
        expect(
          hasFromTo(url),
          "events tab request should include from_/to",
        ).to.eq(true);
        validFromTo(url);
      });
    });
  ...
  it("events listing have valid due date and unit name", () => {
    ...
    cy.contains(/Due Date:\s*\d{2}\/\d{2}\/\d{4},\s*\d{2}:\d{2}/).should(
      "exist",
    );
  });
    ...
    const clickNav = (title: string) => {
      cy.then(() => {
        ...
        cy.then(() => {
          expect(
            eventsCalls.length,
            `events should refetch after clicking ${title}`,
          ).to.eq(before + 1);
        });
        ...
        cy.wait("@getEvents").then((i) => {
          ...
          expect(
            hasFromTo(url),
            `${title} request should include from_/to`,
          ).to.eq(true);
          validFromTo(url);
        });
      });
  ...
  it("jump to date opens calendar and selecting a date triggers re-fetch", () => {
    cy.then(() => {
      ...
      cy.then(() => {
        expect(
          eventsCalls.length,
          "events calls after jump-to-date",
