...
import {Student} from "@/components/analytics/analytics-panel";
...
export function StudentDetailCard({
    ...
    const avg = data.length === 0 ? 0 : Math.round(data.reduce((sum, d) => sum + d.score, 0) / data.length)
            ...
            <Card className={"w-full ${className ?? ''}"}>
                ...
                <CardContent className="space-y-4">
                    <div className={"flex flex-wrap items-center gap-4"}>
                        <div className={"text-xl font-semibold"}>Student Name: {student.name}</div>
                        <div className="flex flex-wrap gap-2">
                            <span className="px-3 py-1 rounded-full bg-accent text-sm">Min — {min}</span>
                            <span className="px-3 py-1 rounded-full bg-accent text-sm">Avg — {avg}</span>
                            <span className="px-3 py-1 rounded-full bg-accent text-sm">Max — {max}</span>
                            <span className="px-3 py-1 rounded-full bg-accent text-sm">N=—{data.length}</span>
                        </div>
                    </div>
