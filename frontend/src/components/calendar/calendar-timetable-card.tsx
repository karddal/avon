...
function TimeGridBody({
  ...
  const { dayHeight, hourStepPx, gridTemplateColumns, gridBackgroundStyle } =
    useTimeGrid({
  ...
  const showNow =
    !!today && days.some((d) => format(d, "yyyy-MM-dd") === today);
  const scrollRef = useRef<HTMLDivElement>(null);
  const gridRef = useRef<HTMLDivElement>(null);
  useScrollToNowCenter(
    scrollRef,
    nowTop,
    dayHeight,
        ...
        <div ref={gridRef} className="grid" style={{ gridTemplateColumns }}>
          ...
          {days.map((day) => {
            ...
            const groups = new Map<
              string,
              { visible: LayoutedEvent[]; hidden: LayoutedEvent[] }
            >();
            ...
            const visibleEvents = Array.from(
              groups.values().flatMap((group) => group.visible),
            );
            ...
            return (
              ...
              >
                ...
                {/* event */}
                {visibleEvents.map(
                  ({ event, top, height, leftPct, widthPct }) => {
                    ...
                    if (href) {
                      return (
                        <Link key={event.id} {...props} href={href}>
                          {content}
                        </Link>
                      );
                    }
                ...
                {Array.from(groups.entries()).map(([groupId, group]) => {
                  ...
                  const groupBottom = Math.max(
                    ...[...group.visible, ...group.hidden].map(
                      (event) => event.top + event.height,
                    ),
                  );
...
function useScrollToNowCenter<T extends HTMLElement>(
  ...
  useEffect(() => {
    ...
    const viewport = ref.current.querySelector<T>(
...
function NowLine({
  ...
  return (
    ...
    >
      <div
        className="absolute right-0 h-[2px] bg-red-500 dark:bg-red-400"
        style={{ left: gutterWidth }}
      />

      <div
        className="absolute -left-[5px] -top-[5px] h-[10px] w-[10px] rounded-full bg-red-500 dark:bg-red-400"
        style={{ left: gutterWidth - 5 }}
      />
  ...
  return useMemo(() => {
    ...
    const layoutDay = (list: CalendarEvent[]): LayoutedEvent[] => {
      ...
      const removeEnded = (startM: number) => {
        active = active.filter((activeEvent) => activeEvent.endM > startM);
      };

      const getUsedCols = () =>
        new Set(active.map((activeEvent) => activeEvent.colIndex));

      for (const item of items) {
        if (active.length > 0) {
          const earliestEnd = Math.min(
            ...active.map((activeEvent) => activeEvent.endM),
          );
