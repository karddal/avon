...
import { Container, FileCheck, Layers } from "lucide-react";
...
export default function ConfigureCWTesting({
  ...
  const [config, setConfig] = useState<DockerConfig>({
    baseImage: img,
    finalCommands: cw_engine_data.tester_command
  ...
  const applyDockerfile = () => {
    ...
    update_coursework_engine(cw_engine_data.cw_id, {
      ...
      tester_command: config.finalCommands,
    }).then((res) => {
      if (res.success) {
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto bg-accent p-4 border rounded-md">
                    <div className="flex gap-2">
                      <Input
                        ...
                        onChange={(e) => updateFinalCommand(e.target.value)}
