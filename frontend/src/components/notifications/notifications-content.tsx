...
export default async function NotificationsContents() {
  ...
  const tabs = groups.map((group) => (
    <TabsTrigger
      className={"whitespace-normal! flex flex-col"}
      key={group.unit_id}
      value={group.unit_id}
    >
      ...
      {group.notifications.filter(
        ...
        <span
          className={
            "absolute -top-2 -right-2 h-5 w-5 rounded-full bg-red-500 flex justify-center items-center items"
          }
        >
          {
            group.notifications.filter(
  ...
  const system_entries =
    data.system_notifications.length === 0 ? (
      <Empty>
        <EmptyHeader>
          ...
          >
            <TabsList className={"md:max-w-1/3"}>
              ...
              >
                ...
                {data.system_notifications.filter(
                  (notification) => notification.viewed === false,
                ).length > 0 && (
                  <span
                    className={
            ...
            <TabsContent value={"system"}>{system_entries}</TabsContent>
            {entries}
          </Tabs>
        </div>
      )}
