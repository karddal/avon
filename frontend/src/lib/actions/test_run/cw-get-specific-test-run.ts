...
export async function get_test_run_for_cw({run_id, cw_id}: {run_id: string, cw_id: string}): Promise<TestRunFullDetails> {
    ...
    if (!response.ok) {
        throw new Error(`Failed to fetch test run data: ${(data as ServerFailedResponse).detail}`);
    } else {
